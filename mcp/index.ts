...
server.tool(
  ...
  "Get the full CII computation audit trail for a country — every score change with the rule version, input data lineage IDs, component breakdown, confidence level, applied rules, and data gaps. Use this to verify WHY a country's score changed and what data drove it. This is the trust/provenance layer.",
  {
    country_code: z
      .string()
      .length(2)
      .describe("ISO 3166-1 alpha-2 country code (e.g. 'UA', 'IR')"),
    days: z
      .number()
      .int()
      .min(1)
...
server.tool(
  ...
  "Cross-reference sanctions events with conflict data and crisis triggers for a specific country. Links OFAC/UN sanctions changes (entity additions, removals, updates) with ACLED conflict events and active crisis playbooks. Use this to understand the sanctions-conflict nexus for a country.",
  {
    country_code: z
      .string()
      .length(2)
      .describe("ISO 3166-1 alpha-2 country code (e.g. 'RU', 'IR', 'KP')"),
    days: z
      .number()
      .int()
      .min(1)
...
server.tool(
  ...
  "Check how well NexusWatch's scenario predictions match reality. Compares predicted CII impacts from scenario simulations against actual CII movements. Returns per-country predicted vs actual scores, mean absolute error, and a tracking status ('aligned', 'partial', 'diverging'). This is the public accountability layer — NexusWatch publishes its track record.",
  {
    scenario_id: z
      ...
      .optional()
      .describe(
        "Scenario ID to validate (e.g. 'hormuz-closure'). Omit for all scenarios."
      ),
    days: z
      .number()
      .int()
    ...
    contents: [
      {
        ...
        text: JSON.stringify(
          {
            scenarios: [
              { id: "hormuz-closure", name: "Strait of Hormuz Closure", triggers: ["Iran military action", "mine deployment", "IRGC naval blockade"], primary_impact: "JP, KR, IN, CN, DE, FR" },
              { id: "taiwan-blockade", name: "China Naval Blockade of Taiwan", triggers: ["PLA Navy deployment", "air defense zone expansion", "trade embargo"], primary_impact: "TW, JP, KR, US, DE" },
              { id: "suez-disruption", name: "Suez Canal Disruption", triggers: ["vessel grounding", "Houthi attacks", "Egyptian political crisis"], primary_impact: "DE, FR, IT, ES, GB, EG" },
              { id: "russia-nato", name: "Russia-NATO Escalation", triggers: ["direct military confrontation", "Article 5 invocation", "nuclear threat"], primary_impact: "PL, RO, DE, FR, GB" },
              { id: "nk-nuclear", name: "North Korea Nuclear Test", triggers: ["underground detonation", "ICBM launch", "test announcement"], primary_impact: "KR, JP, US, CN" },
              { id: "istanbul-earthquake", name: "Istanbul M7+ Earthquake", triggers: ["seismic event M7.0+", "North Anatolian Fault rupture"], primary_impact: "TR, DE, FR, GB" },
              { id: "tehran-earthquake", name: "Tehran M7+ Earthquake", triggers: ["seismic event M7.0+"], primary_impact: "IR, IQ, AF, PK" },
            ],
          },
          null,
          2
