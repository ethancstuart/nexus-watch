...
async function geocodeLocation(location: string): Promise<{ lat: number; lon: number } | null> {
  ...
  try {
    ...
    if (kvUrl && kvToken) {
      try {
        await fetch(
          `${kvUrl}/set/${encodeURIComponent(kvKey)}/${encodeURIComponent(JSON.stringify(result))}/ex/604800`,
          {
            headers: { Authorization: `Bearer ${kvToken}` },
          },
        );
...
async function fetchCategory(category: string, parser: Parser, customSources?: FeedSource[]): Promise<Article[]> {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (categoriesParam) {
    ...
    const categoryResults = await Promise.allSettled(requested.map((cat) => fetchCategory(cat, parser)));
  ...
  return res.setHeader('Cache-Control', 'max-age=600').json({ articles: unique, category, fetchedAt: Date.now() });
}
