 ...
 *
 * Output: 1200x630 PNG in the Light Intel Dossier identity - ivory page,
 * graphite ink, gold top rule, oxblood reserved for misses. Palette IMPORTED
 * from src/styles/email-tokens.ts (rule 8: one brand source), the same module
 * the SSR shell and the email read.
...
function ciiColor(score: number): string {
  // Dossier semantics: severe wears the miss colour, calm wears the up green,
  // the middle stays ink - a number, not a traffic light.
  if (score >= 70) return C.accent;
  if (score >= 50) return C.textPrimary;
  if (score >= 30) return C.textSecondary;
  return C.up;
}
...
// Fonts are BUNDLED (api/_fonts, OFL - see its README) and registered with
// Satori below; without a `fonts` option every stack silently falls back to
// Satori's default face. Family names here must match the registered names.
...
// Edge runtime: no fs. `fetch(new URL(...))` is the documented pattern -
// the bundler inlines the asset. LAZY and memoized, not module-scope: a
// module-scope fetch fires the moment anything imports a render function
// (the unfurl tests do), and a promise rejected at import time is an
// unhandled rejection nothing ever awaits. First handler call pays once per
// isolate; every later call reuses it.
...
function loadFonts(): Promise<ArrayBuffer[]> {
  fontData ??= Promise.all([
    ...
    fetch(new URL('./_fonts/SourceSerif4-Semibold.ttf', import.meta.url)).then((r) => r.arrayBuffer()),
  ]);
  return fontData;
}

function renderSiteCard(): string {
  return `<div style="display:flex;flex-direction:column;justify-content:space-between;width:100%;height:100%;background:${C.bgPage};border-top:8px solid ${C.divider};padding:56px 72px 64px 72px;font-family:${MONO};">
    <div style="display:flex;align-items:center;justify-content:space-between;">
      <span style="color:${C.textSecondary};font-size:22px;font-weight:700;letter-spacing:0.18em;">NEXUSWATCH</span>
...
function renderBriefCard(date: string, headline: string, tension: number | null): string {
  const tColor = tension == null ? C.textTertiary : ciiColor(tension);
  const tLabel = tension == null ? 'TENSION INDEX' : `TENSION ${tension}`;
  const safeHeadline = escapeHtml(headline.slice(0, 140));
  return `<div style="display:flex;flex-direction:column;justify-content:space-between;width:100%;height:100%;background:${C.bgPage};border-top:8px solid ${C.divider};padding:56px 72px 64px 72px;font-family:${MONO};">
    <div style="display:flex;align-items:center;justify-content:space-between;">
      <span style="color:${C.textSecondary};font-size:18px;font-weight:700;letter-spacing:0.18em;">NEXUSWATCH · BRIEF</span>
      <span style="color:${C.textInverse};background:${C.up};font-size:11px;font-weight:700;letter-spacing:0.18em;padding:5px 12px;border-radius:999px;">FREE.</span>
    </div>
    <div style="display:flex;flex-direction:column;gap:18px;">
      <span style="color:${C.textTertiary};font-size:18px;letter-spacing:0.14em;">SITREP · ${escapeHtml(date)}</span>
      <span style="color:${C.textPrimary};font-size:46px;font-weight:600;line-height:1.2;font-family:${SERIF};">${safeHeadline}</span>
    </div>
    <div style="display:flex;justify-content:space-between;align-items:center;">
      <span style="color:${tColor};font-size:16px;font-weight:700;letter-spacing:0.14em;">${tLabel}</span>
      <span style="color:${C.textTertiary};font-size:14px;">nexuswatch.dev/brief/${escapeHtml(date)}</span>
    </div>
  </div>`;
}

function renderCountryCard(iso: string, name: string, score: number | null): string {
  const color = score == null ? '${C.textTertiary}' : ciiColor(score);
  const label = score == null ? 'NO DATA' : ciiLabel(score);
  const display = score == null ? '—' : String(score);
  const emoji = flag(iso);
  return `<div style="display:flex;flex-direction:column;justify-content:space-between;width:100%;height:100%;background:${C.bgPage};border-top:8px solid ${C.divider};padding:56px 72px 64px 72px;font-family:${MONO};">
    <div style="display:flex;align-items:center;justify-content:space-between;">
      <span style="color:${C.textSecondary};font-size:18px;font-weight:700;letter-spacing:0.18em;">NEXUSWATCH · COUNTRY</span>
    ...
    <div style="display:flex;flex-direction:column;gap:8px;">
      <div style="display:flex;align-items:center;gap:24px;">
        ...
        <span style="color:${C.textPrimary};font-size:56px;font-weight:600;font-family:${SERIF};">${escapeHtml(name)}</span>
      </div>
      <div style="display:flex;align-items:baseline;gap:18px;">
        <span style="color:${color};font-size:128px;font-weight:700;font-family:${MONO};line-height:1;">${display}</span>
        <span style="color:${C.textTertiary};font-size:18px;letter-spacing:0.14em;">CII · 0-100</span>
      </div>
    </div>
    <div style="display:flex;justify-content:space-between;align-items:center;">
      <span style="color:${C.textInverse};background:${C.up};font-size:11px;font-weight:700;letter-spacing:0.18em;padding:5px 12px;border-radius:999px;">FREE.</span>
      <span style="color:${C.textTertiary};font-size:14px;">nexuswatch.dev/country/${escapeHtml(iso)}</span>
    </div>
  </div>`;
}
...
function statusColor(status: string): string {
  // Oxblood is RESERVED for misses (identity ruling): the only red on the
  // register is where we were wrong. Hits get the market-up green.
  if (status === 'hit') return C.up;
  if (status === 'miss') return C.accent;
...
export function renderLedgerCard(f: LedgerFacts | null): string {
  ...
  return `<div style="display:flex;flex-direction:column;justify-content:space-between;width:100%;height:100%;background:${C.bgPage};border-top:8px solid ${C.divider};padding:56px 72px 64px 72px;font-family:${MONO};">
    <div style="display:flex;align-items:center;justify-content:space-between;">
      <span style="color:${C.textSecondary};font-size:18px;font-weight:700;letter-spacing:0.18em;">NEXUSWATCH · THE LEDGER</span>
      <span style="color:${C.textInverse};background:${C.up};font-size:11px;font-weight:700;letter-spacing:0.18em;padding:5px 12px;border-radius:999px;">FREE.</span>
    </div>
    <div style="display:flex;flex-direction:column;gap:22px;">
      <span style="color:${C.textPrimary};font-size:44px;font-weight:600;line-height:1.15;font-family:${SERIF};">Every call we make, scored against something that isn’t us.</span>
      <div style="display:flex;align-items:flex-end;gap:64px;">
        <div style="display:flex;flex-direction:column;gap:6px;">
          <span style="color:${C.textPrimary};font-size:96px;font-weight:700;line-height:1;font-family:${MONO};">${leftValue}</span>
          <span style="color:${C.textTertiary};font-size:15px;letter-spacing:0.14em;">CALLS OPEN</span>
        </div>
        <div style="display:flex;flex-direction:column;gap:6px;">
          <span style="color:${C.up};font-size:96px;font-weight:700;line-height:1;font-family:${MONO};">${rightValue}</span>
          <span style="color:${C.textTertiary};font-size:15px;letter-spacing:0.14em;">${rightLabel}</span>
        </div>
      </div>
    </div>
    <div style="display:flex;justify-content:space-between;align-items:center;">
      <span style="color:${C.textTertiary};font-size:14px;letter-spacing:0.08em;">resolved by an external source, on a date fixed in advance</span>
      <span style="color:${C.textTertiary};font-size:14px;letter-spacing:0.08em;">${foot}</span>
    </div>
  </div>`;
}
...
export function renderCallCard(c: CallFacts): string {
  ...
  return `<div style="display:flex;flex-direction:column;justify-content:space-between;width:100%;height:100%;background:${C.bgPage};border-top:8px solid ${C.divider};padding:56px 72px 64px 72px;font-family:${MONO};">
    <div style="display:flex;align-items:center;justify-content:space-between;">
      <span style="color:${C.textSecondary};font-size:18px;font-weight:700;letter-spacing:0.18em;">NEXUSWATCH · CALL #${c.id}</span>
    ...
    <div style="display:flex;flex-direction:column;gap:20px;">
      <span style="color:${C.textTertiary};font-size:18px;letter-spacing:0.14em;">${escapeHtml(c.countryCode)} · STATED ${escapeHtml(c.madeOn)}</span>
      <span style="color:${C.textPrimary};font-size:40px;font-weight:600;line-height:1.2;font-family:${SERIF};">${claim}</span>
      <div style="display:flex;align-items:baseline;gap:20px;">
        <span style="color:${C.textSecondary};font-size:104px;font-weight:700;line-height:1;font-family:${MONO};">${stated}</span>
        <span style="color:${C.textTertiary};font-size:18px;letter-spacing:0.14em;">WHAT WE SAID</span>
      </div>
    </div>
    <div style="display:flex;justify-content:space-between;align-items:center;">
      <span style="color:${color};font-size:16px;font-weight:700;letter-spacing:0.14em;">${resolutionLine}</span>
      <span style="color:${C.textTertiary};font-size:14px;">nexuswatch.dev/call/${c.id}</span>
    </div>
  </div>`;
}
...
export default async function handler(req: VercelRequest) {
  ...
  const [monoRegular, monoBold, serifSemibold] = await loadFonts();
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const response = new ImageResponse(htmlToSatori(html) as any, {
    width: 1200,
    height: 630,
    fonts: [
