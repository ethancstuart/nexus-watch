...
const COUNTRY_COORDS: Record<string, [number, number]> = {
  ...
  'India': [20.6, 78.9], 'North Korea': [40.0, 127.0], 'Saudi Arabia': [24.7, 46.7],
};
...
const DEMONYMS: Record<string, string> = {
  ...
  'Nigerian': 'Nigeria', 'Somali': 'Somalia', 'Pakistani': 'Pakistan', 'Turkish': 'Turkey',
};
...
function classifyConflict(title: string): { type: string; subType: string; fatalities: number } {
  ...
  if (lower.includes('airstrike') || lower.includes('bombing')) return { type: 'Battles', subType: 'Air/drone strike', fatalities };
  if (lower.includes('missile') || lower.includes('rocket')) return { type: 'Battles', subType: 'Shelling/missile', fatalities };
  if (lower.includes('protest') || lower.includes('demonstrat')) return { type: 'Protests', subType: 'Protest', fatalities };
  if (lower.includes('attack') || lower.includes('offensive')) return { type: 'Battles', subType: 'Armed clash', fatalities };
  if (lower.includes('ceasefire') || lower.includes('truce')) return { type: 'Strategic developments', subType: 'Ceasefire', fatalities: 0 };
...
function extractCountry(title: string): { country: string; lat: number; lon: number } | null {
  ...
  for (const [dem, country] of Object.entries(DEMONYMS)) {
    if (title.includes(dem)) { const c = COUNTRY_COORDS[country]; return c ? { country, lat: c[0], lon: c[1] } : null; }
  }
  return null;
}
...
export default async function handler(_req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (rows.length === 0 || !rows[0].data) {
      return res.json({ events: [], count: 0, source: 'gdelt-conflict', error: 'Cache empty — cron populates every 5 min' });
    }

    const cached = rows[0].data as { articles?: Array<{ title: string; source: string; sourcecountry: string; seendate: string }> };
    ...
    const events = (cached.articles || [])
      .map((a) => {
        ...
        return {
          id: `gdelt-${seen.size}`, date: a.seendate?.slice(0, 8).replace(/(\d{4})(\d{2})(\d{2})/, '$1-$2-$3') || '',
          type: c.type, subType: c.subType, actor1: a.source || '', actor2: '',
          country: geo.country, region: '', lat: geo.lat + (Math.random() - 0.5) * 1.5,
          lon: geo.lon + (Math.random() - 0.5) * 1.5, fatalities: c.fatalities,
    ...
    return res.setHeader('Cache-Control', 'public, max-age=300, s-maxage=300').json({
      events, count: events.length, source: 'gdelt-conflict', cachedAt: rows[0].updated_at,
    });
  } catch (err) {
