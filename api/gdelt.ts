...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const params = new URLSearchParams({
      ...
      mode: 'artlist', maxrecords, timespan: `${timespan}min`, format: 'json', sort: 'DateDesc',
    });
    ...
    const data = (await response.json()) as {
      articles?: { title: string; url: string; source: string; sourcecountry: string; tone: number; socialimage: string; domain: string; language: string; seendate: string }[];
    };

    const articles = (data.articles || []).map((a) => ({
      title: a.title, url: a.url, source: a.source, sourceCountry: a.sourcecountry || '',
      tone: typeof a.tone === 'number' ? a.tone : 0, domain: a.domain || '',
      language: a.language || 'English', image: a.socialimage || '', date: a.seendate || '',
    }));
