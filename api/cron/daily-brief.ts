...
export default async function handler(_req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    let flightCount = 0;
    try {
      const fRes = await fetch('https://api.adsb.lol/v2/lat/40/lon/-74/dist/250', {
        signal: AbortSignal.timeout(8000),
      });
      if (fRes.ok) {
        const fData = (await fRes.json()) as { ac?: unknown[] };
        flightCount = fData.ac?.length || 0;
      }
    } catch {
      /* */
    }
    ...
    try {
      const cRes = await fetch(
        'https://api.gdeltproject.org/api/v2/doc/doc?query=attack%20OR%20airstrike%20OR%20ceasefire%20OR%20war&mode=artlist&maxrecords=10&timespan=1440min&format=json&sort=DateDesc',
        { signal: AbortSignal.timeout(10000) },
      );
      if (cRes.ok) {
    ...
    try {
      ...
      if (apiKey) {
        const mRes = await fetch(`https://api.twelvedata.com/quote?symbol=SPY,GLD,USO,UUP&apikey=${apiKey}`, {
          signal: AbortSignal.timeout(8000),
        });
        if (mRes.ok) {
          const mData = (await mRes.json()) as Record<
            string,
            { close?: string; percent_change?: string; name?: string }
          >;
          const lines: string[] = [];
          for (const [sym, q] of Object.entries(mData)) {
            if (q?.close)
              lines.push(
                `${sym}: $${parseFloat(q.close).toFixed(2)} (${parseFloat(q.percent_change || '0') > 0 ? '+' : ''}${parseFloat(q.percent_change || '0').toFixed(2)}%)`,
              );
          }
        ...
        if (allEmails.size > 0) {
          ...
          await fetch('https://api.resend.com/emails', {
            ...
            body: JSON.stringify({
              ...
              <div style="font-size: 13px; line-height: 1.8; white-space: pre-wrap;">${summary
                .replace(/</g, '&lt;')
                .replace(/>/g, '&gt;')
                .replace(/\n/g, '<br>')
                .replace(/## /g, '<br><strong style="color:#ff6600">')
...
## KEY DEVELOPMENTS
${
  data.conflictHeadlines
    .slice(0, 5)
    .map((h) => `▸ ${h} (MODERATE confidence)`)
    .join('\n') || '▸ No major conflict developments detected'
}

## TOP RISK COUNTRIES
${data.topRiskCountries
  .slice(0, 5)
  .map((c) => `- **${c.name}**: CII ${c.score}/100`)
  .join('\n')}
