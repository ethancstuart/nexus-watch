...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const tf = validTimeframes[timeframe] || 'day';

  try {
    const url = `https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_${tf}.geojson`;
    const response = await fetch(url);
    if (!response.ok) {
      return res.status(response.status).json({ error: 'USGS API error' });
    }
    ...
    return res
      ...
      .json({ earthquakes, count: earthquakes.length, generated: data.metadata.generated });
  } catch (err) {
