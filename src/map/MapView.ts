...
/**
 * The maplibre stylesheet is BUNDLED, not fetched.
 *
 * It used to be pulled from `unpkg.com/maplibre-gl@latest` at runtime —
 * unpinned, third-party, and executing on this origin, so whoever published
 * that package next controlled CSS on every page that opened the map. The
 * package ships its own stylesheet; importing it lets Vite hash and serve it
 * from our origin, which closes the hole and removes unpkg from the content
